const IMAGES_JSON = '/birthday/images.json';
let images = [];
let index = 0;
let playing = false;
...
// Birthday counter: December 8, 2001 (birth date)
...
async function tryDirectoryListing(){
  ...
  try{
    const r = await fetch('/birthday/images/');
...
function normalizeEntries(arr){
  ...
  return arr.map(p=>{
    if(p.startsWith('/birthday/')) return p;
    if(p.startsWith('http')) return p;
    if(p.startsWith('images/')) return '/birthday/'+p;
    if(p.startsWith('/')) return p;
    return '/birthday/images/'+p;
  });
}
...
function updateUptime(){
  const now = new Date();
  const elapsed = now - BIRTHDAY_DATE;
  const isNegative = elapsed < 0;
  const absElapsed = Math.abs(elapsed);
  
  // Calculate years, months, days, hours, minutes
  let years = 0, months = 0, days = 0;
  let current = new Date(BIRTHDAY_DATE);
  const target = isNegative ? now : new Date(BIRTHDAY_DATE);
  const endpoint = isNegative ? BIRTHDAY_DATE : now;
...
async function init(){
  ...
  if(!list || !list.length){
    document.getElementById('caption').textContent = 'No images found. Put files into /birthday/images/ and run generate_images_json.ps1 to create images.json for best results.';
    return;
  }
